...
3.5 通知・ロック画面操作
通知内アクション: ロック画面に表示される通知上に「次へ」「巻き」のボタンを配置し、アプリを開かずに制御可能とする。

触覚フィードバック: フェーズ終了時および「巻き」実行時に、特定のパターンで振動させる。

4. UI/UX デザイン仕様
   デザインコンセプト: 視認性重視。文字情報よりも「色」と「大きさ」を優先する。

ボタン設計: 授業中の慌ただしい状況を想定し、タップ有効範囲を最大化（巨大なボタン）。
